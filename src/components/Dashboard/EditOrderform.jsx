...
import { isOpenEditOrder } from '../../context/slices/editOrderSlice';
        ...
        <div className="fixed top-0 w-screen h-screen bg-black/80 flex items-center justify-center z-50">
            <div className="bg-gray-300 drop-shadow-xl rounded-xl sm:w-1/2 w-full mx-2" style={{ maxWidth: '500px' }}>
                <div className="w-full borderCardChrono relative px-8 py-4 flex items-center">
                    <span className="text-gray-900 font-bold text-lg">Edit Order</span>
                    <button onClick={() => dispatch(isOpenEditOrder())} className="absolute right-0 mr-5 cursor-pointer">
                ...
                <div className="flex flex-col h-full p-7">
                    <form onSubmit={handleEditColumn} className="flex flex-col w-full">
                        <div className="flex flex-col relative my-2">
                            <label htmlFor="referance" className="mb-2 text-base bg-transparent font-semibold">Referance</label>
                            <div className="outlin relative w-full">
                                <input id="referance" type="text" name="referance" placeholder="Referance" className="block p-4 w-full inputLogin appearance-none outline-none text-sm font-medium" />
                            </div>
                        ...
                        <div className="flex flex-col relative my-2">
                            <label htmlFor="price" className="mb-2 text-base bg-transparent font-semibold">Price</label>
                            <div className="outlin relative w-full">
                                <input id="price" type="text" name="price" placeholder="Price" className="block p-4 w-full inputLogin appearance-none outline-none text-sm font-medium" />
                            </div>
                            {displayError('price')}

                        </div>
                        <div className="flex flex-col relative my-2">
                            <label htmlFor="deliver" className="mb-2 text-base bg-transparent font-semibold">Deliver</label>
                            <select className="block p-4 w-full inputLogin appearance-none outline-none text-sm font-medium">
                                <option value="yes">Yes</option>
                                <option value="no">No</option>
                            </select>
                            {displayError('deliver')}

                        </div>
                        <div className="flex flex-row w-full place-content-end mt-4">
                            <button onClick={() => dispatch(isOpenEditOrder())} className="py-3 px-4 mt-2 mr-6 rounded font-medium text-sm cursor-pointer border bg-white hover:bg-pink-500 hover:text-white">Cancel</button>
